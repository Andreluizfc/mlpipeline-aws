...
def gated_roi_pooling(input, rois, gates=None, size=ROI_POOL_SIZE, spatial_scale=1.0):
    ...
    for i in range(num_rois):
        roi = rois[i]
        im_idx = roi[0]
        im = input.narrow(0, im_idx, 1)[..., roi[2]:(
            roi[4] + 1), roi[1]:(roi[3] + 1)]
...
class FashionNetVgg16NoBn(nn.Module):
    def __init__(self):
        ...
        self.fc6_global = nn.Linear(
            in_features=512 * 7 * 7, out_features=1024 * 4)
    ...
    def forward(self, x):
        ...
        fc6_global = F.leaky_relu(self.fc6_global(
            self.flatten(self.conv5_global(base_features))))
